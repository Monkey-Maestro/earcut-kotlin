...
class Point3(var x: Double = 0.0, var y: Double = 0.0, var z: Double = 0.0){
    ...
    public operator fun minus(v2: Point3): Point3 {
        return Point3(x - v2.x, y - v2.y, z - v2.z)
    }

    public operator fun plus(v2: Point3): Point3 {
        return Point3(v2.x + x, v2.y + y, v2.z + z)
    }

    public operator fun times(sc: Double): Point3 {
        return Point3(x * sc, y * sc, z * sc)
    }

    public operator fun div(sc: Double): Point3 {
        return Point3(x / sc, y / sc, z / sc)
    }
...
fun normal(vertices : DoubleArray): DoubleArray {
    ...
    for (i in points.indices) {
        ...
        if(!ccw)
            c123 = c123 + v32.cross(v12)
        else
            c123 = c123 + v12.cross(v32)
