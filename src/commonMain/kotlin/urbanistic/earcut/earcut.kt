...
val xComparator = Comparator { a: Node, b: Node -> compareValues(a.x, b.x)}
    ...
    private fun eliminateHoles(data: DoubleArray, holeIndices: IntArray, _outerNode: Node, dim: Int) : Node {
        ...
        while (i < len) {
            ...
            getLeftmost(list)?.let { queue.add(it) }
            i++
        }

        //This line broke native Tests, therfore a bit more complex :)
        //queue = queue.sortedWith(xComparator) as MutableList<Node>
        val queue_ = queue.sortedWith(xComparator)
        queue.clear()
        queue.addAll(queue_)
    ...
    private fun eliminateHole(hole: Node, _outerNode: Node) {
        //var outerNode: Node = _outerNode
        val outerNode : Node? = findHoleBridge(hole, _outerNode)
        if (outerNode != null) {
            val b: Node = splitPolygon(outerNode, hole)
    ...
    private fun locallyInside(a: Node, b: Node): Boolean {
        return if (area(a.prev, a, a.next) < 0) area(a, b, a.next) >= 0 && area(a, a.prev, b) >= 0
